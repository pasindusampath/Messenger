...
public class MainThread extends Task<Message> {

    @Override
    protected Message call() throws Exception {
        ServerSocket server = ServerConnection.getServerConnection().getServer();
                    ...
                    public void run() {
                        ...
                        while (true) {
                            try {
                                ...
                                if (s.equals("<p><vid>")) {
                                    updateValue(new Message("start",null));
                                    System.out.println("started");
                                    String signal;
                                    do {
                                        signal = inFormClient.readUTF();
                                        if(signal.equalsIgnoreCase("end")){
                                            updateValue(new Message("end",null));
                                            break;
                                        }
                                        int size = inFormClient.readInt();
                                        //System.out.println();
                                        ...
                                        updateValue(new Message("vid",read));
                                    }while (true);
                                    continue;
                                }
                                try {
